...
{
    public partial class FrmUserType : DevExpress.XtraEditors.XtraForm
    {
        int Id = 0;
        bool SaveFlag = true;
        int count = 0;
        ...
        private void FillGrid()
        {
            try
            {
                DataTable dt = new UserType_Function().GetDataFoGrid("");
                if (dt.Rows.Count > 0)
                {
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                UserType_Property ObjProperty = new UserType_Property();
                ObjProperty.Id = Id;
                ObjProperty.Type = txtUserType.Text;

                int RetVal = new UserType_Function().InsertUpdate(ObjProperty);
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Id > 0)
                {
                    int RetVal = new UserType_Function().Delete(Id);
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (gridView1.RowCount > 0)
                {
                    ...
                    txtUserType.Text = Convert.ToString(gridView1.GetFocusedRowCellValue("USER TYPE"));
                }
            }
            catch (Exception ex)
            {
