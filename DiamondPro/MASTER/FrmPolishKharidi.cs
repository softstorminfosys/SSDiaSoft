...
{
    public partial class FrmPolishKharidi : DevExpress.XtraEditors.XtraForm
    {
        int Id = 0;
        bool SaveFlag = true;

        #region Constructor
        public FrmPolishKharidi()
        {
            InitializeComponent();
        }
        #endregion
        ...
        private void txtDalalName_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                if (e.KeyChar != 13 && e.KeyChar != 8)
                {
                    DataTable dtParty = new MST_Dalal_Function().DalalMasterGetDataFOrGrid(" dbo.MST_Dalal.PartyId = " + Convert.ToInt32(txtPartyName.Tag) + " AND  MST_Dalal.ACTIVE = 1");
                    FrmSearchProperty frmSearch = new FrmSearchProperty();
                    frmSearch.dtTable = dtParty.Copy();
        ...
        private void txtMarginPer_TextChanged(object sender, EventArgs e)
        {
            CalculateMarginPer();
        }

        private void txtMarginAmt_TextChanged(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = new MST_Kharidi_Function().KharidiMasterGetNotNo(1);
            if (dt.Rows.Count == 1)
            {
        ...
        private void FillGrid()
        {
            try
            {
                DataTable dt = new MST_PolishKharidi_Function().PolishKharidiMasterGetDataFOrGrid("");
                if (dt.Rows.Count > 0)
                {
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MST_PolishKharidi_Property ObjProperty = new MST_PolishKharidi_Property();
                ObjProperty.ID = Id;
                ObjProperty.notno = txtNotNo.Text;
                ObjProperty.katno = txtKatNo.Text;
                ObjProperty.KharidiDate = DateTime.Parse(dtpKharidiDate.DateTime.ToString()).ToString("yyyy/MM/dd");
                ObjProperty.PaymentDate = DateTime.Parse(dtpPaymentDate.DateTime.ToString()).ToString("yyyy/MM/dd");
                ...
                int RetVal = new MST_PolishKharidi_Function().InsertUpdate(ObjProperty);
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Id > 0)
                {
                    int RetVal = new MST_PolishKharidi_Function().Delete(Id);
